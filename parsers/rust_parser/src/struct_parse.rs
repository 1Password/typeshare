...
use typeshare_core::parsed_types::{
    Comment, CommentLocation, Field, Item, ParsedStruct, ParsedTypeAlias, Source, StructShared,
};
...
pub fn parse_struct(s: &ItemStruct, source: Source) -> Result<Item, ParseError> {
    ...
    let generic_types = ParseGenerics::from_syn_generics(&s.generics).0;
