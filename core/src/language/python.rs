...
// itself
fn collect_generics_for_variant(variant_type: &RustType, generics: &Vec<String>) -> Vec<String> {
    let mut all = vec![];
    match variant_type {
...
impl Language for Python {
    ...
    fn begin_file(&self, w: &mut dyn Write) -> std::io::Result<()> {
        ...
        for (import_module, identifiers) in &module.imports {
            let mut identifier_vec = identifiers
                .iter()
                .cloned()
                .collect::<Vec<String>>();
                identifier_vec.sort();
    ...
    fn write_struct(&self, w: &mut dyn Write, rs: &RustStruct) -> std::io::Result<()> {
        {
            ...
            rs.generic_types
                .iter()
                .cloned()
                .for_each(|v| module.add_type_var(v))
        }
                    ...
                    match variant {
                            ...
                            match ty {
                                RustType::Generic { id: _, parameters } => {
                                    ...
                                    let type_vars = self
                                        .module
                                        .borrow_mut()
                                        .get_type_vars(generic_parameters.len());
                        ...
                        RustEnumVariant::AnonymousStruct {
                            ...
                            let type_vars = self
                                .module
                                .borrow_mut()
                                .get_type_vars(num_generic_parameters);
